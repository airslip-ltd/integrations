...
locals {
  ...
  external_apis = var.external_apis
  external_custom_domain = var.external_custom_domain
  external_api_policy = "./variables/${var.short_environment}/policies/external_api_policy.xml"
}
...
module "api_management_ext" {
  ...
  app_configuration = {
    ...
    api_publisher_email = local.api_publisher_email,
    api_sku_name = local.api_sku_name,
    api_custom_domain = local.external_custom_domain,
    tenant_id = data.azurerm_client_config.current.tenant_id,
    deployer_id = local.deployment_agent_group_id,
    revision = local.revision,
    policy = local.external_api_policy
  }

  apis = local.external_apis
}
